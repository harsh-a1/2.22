/**
 * Created by HISP-WS12 on 03-03-2016.
 */
/**
 * Created by HISP-WS12 on 03-03-2016.
 */
...
angular.module('trackerCaptureServices')
    ...
    .service('CustomIDGenerationService',function($http,$q,ProgramFactory,RegistrationService,CustomIdService){
        ...
        return {
            ...
            createCustomId :  function(totalTeiCount,CustomIDPrefix){

                var thisDef = $.Deferred();

                var constantPreFix = CustomIDPrefix;
                var prefix = "";
                //console.log( "total Count -- " + response.data.height);
                //var totalTei = response.data.height + 1;

                var totalTei = totalTeiCount;
                console.log("tei count"+totalTeiCount);
                console.log("prefix"+CustomIDPrefix);
                ...
                var finalCustomId = constantPreFix + prefix + totalTei;
            ...
            createCustomIdAndSave: function(tei,customIDAttribute,optionSets,attributesById,totalTeiCount,orgUnitCode,CustomIDPrefix){
                var def = $.Deferred();
                console.log(  totalTeiCount + "--" + CustomIDPrefix);

                this.createCustomId(totalTeiCount,CustomIDPrefix).then(function(customId){
                        ...
                        if (isValidAttribute && isValidProgram)
                        {
                            ...
                            CustomIdService.getTotalTeiByProgram(programUid).then(function(teiResponse){
                                ...
                                CustomIdService.getOrgunitCode(tei.orgUnit).then(function(orgUnitCodeResponse){
                                    ...
                                    thiz.createCustomIdAndSave(tei,customIDAttribute,optionSets,attributesById,totalTei,orgUnitCode,custIDPrefix).then(function(response){
